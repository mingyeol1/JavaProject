...
//		 3. copyOfRange(원본배열, 시작인덱스, 끝인덱스) : 원본 배열의 시작인덱스에서 끝 인데스까지 복사 한 배열 리턴
//		 	시작인덱스는 포함이지만, 끝 인덱스 포함되지 않음.
		...
		System.out.println("처음부터 3번째 요소까지 배열 복사.");
		for(double d: arrCpy2) {
			System.out.print(d+"\t");
		}
		System.out.println();
		
		//1번 인덱스에서 3번인덱스까지 복사.(특정 범위 내 부분복사.)
		double[] arrCpy3 = Arrays.copyOfRange(argOrg, 1, 4);
		System.out.println("1번 인덱스부터 3번 인덱스(포함)까지 배열복사");
		...
		if(Arrays.equals(scores, scores3)) {
			System.out.println("배열이 각 항목이 모두 일치함");
		}else {
