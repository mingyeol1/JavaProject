...
public class H04_ThreadEx3 {
	 ...
	 * 
	 * [스레드 동기화]
	 * 	여러 스레드가 동작하는 프로그램에서 하나의 객체를 함께 사용하는 경우, 의도치 않는 
	 * 	결과를 가져올 수 있다. 하나의 스레드가 작업이 끝날때까지 객체가 변경되지 않도록
...
//			if(this.level % 10 == 0) break;
			if(this.level == 5) {
				...
				try {
					//notity() : 현재 대기중인 스레드를 한개만 진행 중으로 변환(직접 지정불가,)
					wait();		//헌재 실행 중인 스레드를  대기 상태로 전환
				} catch (InterruptedException e) {}
				break;
			}
			//레벨이 10의 배수가 되면 종료
