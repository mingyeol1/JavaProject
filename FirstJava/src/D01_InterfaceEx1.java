 ...
 * # 인터페이스 
 ...
 인터페이스 선언 
  ...
  interface InterfaceName [extends <SuperInterface>,<SupterInterface>....] {  //상속이 가능 부모 인터페이스를 받을 수 있다. 구현이 자유롭다. 
  		interfaceBody //추상메서드
  }
 ...
 예) 인터페이스 정의
  public interface Aquatic {
  		...
  		public abstract void breathUnderWater(); //인터페이스는 public abstract생략이 가능 //
  }
...
interface ShapeInterface{
	//인터페이스 변수는 public static final 을 생략 할 수 있고, 이는 상수로
	//인터페이스 메서드는 추상 메서드로 public abstract를 생략할 수 있다.
	public abstract double getArea();		//추상 메서드
	//double getAarea(); 로 적어도 상관 없다.	//위와 같다.
}
...
class RactangleI implements ShapeInterface{
	...
	public RactangleI(int width, int height) { //생성자.
		this.width = width;
		this.height = height;
		}
	@Override
...
public class D01_InterfaceEx1 {
	...
	public static void main(String[] args) {
		...
		ShapeInterface s1 = new RactangleI(30,48);
		
		System.out.println("rect's Area = "+rectangleI.getArea());
		System.out.println("triane's Area = "+triangleI.getArea());
