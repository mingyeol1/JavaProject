...
//		Wallet<String, Integer> card_wallet3 = new Wallet<>("파란색"); //상속받지 못했다..
	...
	//	card_wallet2.pouch.add(new CheckCard());
		...
		str_list.add("제너릭 때문에 문자열만 들어갈 수 있다.");
		str_list.add("이유는? 문자열 제너릭 컬렉션으로 지정되기 때문이다.");
		int_list.add(100);
...
// 제너릭에서 extends는 타입의 범위를 제한하는 역할을 하게 됨.
// - One extends Card : 첫번째 타입으로 Card를 상속받는 클래스만 허용
// - Two extends Money : 두번째 타입으로 Money를 상속받는 클래스만 허용.

class Wallet<One extends Card, Two extends Money>{ // One Two는 변수이름. 
