...
public class E07_hashMapEx {
	public static void main(String[] args) {
		...
		// key는 중복 허용 x. 같은 키로 다른 값을 넣으면 갱신처리.
		// key와 value는 1:1 관계
		...
		//값이 없으면  두번 매개변수임 default 값을 가져온다. 
		...
		for(Entry<String, Integer> e:map.entrySet()) {
			System.out.print(e.getKey() + ":"); //e.getKey라는 엔트리는 메서드가 있어 사용이 가능하다.
			System.out.println(e.getValue());
			
			//values() : 모든 값을 Collection 타입으로 반환한다.
			System.out.println(map.values());
			
			//containsKey(key) : 키가 포함 되어 있는지 여부 확인.
