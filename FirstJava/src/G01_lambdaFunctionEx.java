...
public class G01_lambdaFunctionEx {
	...
	static int sum(int x, int y) { //외부에 있는 다른 메서드 
		return x + y;
	}
	//람다식(간결한 코드를 위해서 사용한다.)
	// 고전적인 방식인 객체지향적인 프로그램 방식이 아닌 함수적 프로그래밍을 위한 기법 사용
	// 람다는 익명함수를 위한 방식입니다. 람단ㄴ 기존방식(객체지향)보다 자바코드를 간결하게 표현할 수 있다
	//  는 장점을 가지고 있습니다. 다만, 처음에는 익훅하지 않아 혼란스러울 수 있다.
	
	// 람다식은 함수적 인터페이스를 자료형으로 사용한다.
	// 함수적 인터페이스란? 추상 메서드가 한개만 존제하는 인터페이스를 의미함.
...
//		};
		...
		li4 = (x, y) -> x%y;
		System.out.println("두 수의 나머지 : "+li4.cal(3, 2));
