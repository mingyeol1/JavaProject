...
public class A03_variable {
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
			//변수 (variavble)
			//데이터를 덮어서 보관할 수 있는 공간을 의미함.
			//java에서 변수는 반드시 선언한 뒤에 사용해야한다.
		
			//변수, 메서드, 클래스, 이름을 식별자
			//대문자와 소문자를 구분하고, 길이 제한이 없음, ex) Run, rUn, ruN
			//문자와 숫자, 밑줄(_), 달러기호($)를 포함할 수 있다.
			//문자는 영문자만 사용하는 것은 아니나 영문자 사용을 추천
			//_, $는 로컬 시스템에서 규칙이 있는 경우가 있기 때문에 부득이한 경우 아니면 사용 안함.
			//문자 , 밑줄(_), 달러기호($)로 시작할 수 있다. 숫자로는 시작할 수 없음.
			//식별자에는 공백을 포함할 수 없다. Ex)is child(x)-> isChild, is_child
			//키워드는 식별자로 사용하면 안된다.
			//키워드는 시스템 또는 자바에서 기본적으로 사용하고 있는 명령어나 변수들을 의미한다.
			...
			// > 첫글자는 대문자로 표기한다.
			// > 연걸된 단어의 첫글자는 대문자로 표기한다.
		// > 나머지는 소문자로 표기.
		// > $는 내부클래스에서 특별한 의미를 가지고 있기 때문에 사용을 권장하지 않는다.
		// > ex) CustomerManeger
		
		// - 변수 이름
		// > 명사적 의미를 갖게 만들어 준다.
		// > 첫 글자는 소문자로, 연결되는 단어의 첫글자는 대문자로
		// > 나머지는 소문자.
		// > 일반적으로 변수 이름에선ㄴ "_"를 사용하지 않는다.
		// ex)customerName
		
		
		// - 메서드 이름
		// > 동사적 의미를 갖게 작성한다.
		// > 첫글자는 소문자, 연결단어의 첫 글자는 대문자로 표기
		// > 나머지는 소문자.
		...
		//int a = 20; //같은 이름의 변수는 밑에서 다시 선언 할 수 없다.
		a = 20; //재사용
		// int는 정사 타입을 나타내는 키워드
		...
		//2. 값에 의미를 부여할 수 있다. (이 경우 작명 규칙이 중요함)
		int height = 100;
		int subject = 5;
		int old = 26;
		int weight = 70;
